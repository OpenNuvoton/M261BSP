...
void _WriteM1(U8 * pData, int NumItems)
{
    ...
    /* Set transfer count */
    PDMA_SET_TRANS_CNT(PDMA0, USPI_MASTER_TX_DMA_CH, NumItems);
    /* Set source address */
    PDMA_SET_SRC_ADDR(PDMA0, USPI_MASTER_TX_DMA_CH, (uint32_t)pData++);
    /* Set basic mode */
    PDMA0->DSCT[USPI_MASTER_TX_DMA_CH].CTL = (PDMA0->DSCT[USPI_MASTER_TX_DMA_CH].CTL & ~PDMA_DSCT_CTL_OPMODE_Msk) | PDMA_OP_BASIC;
    ...
    while((PDMA_GET_TD_STS(PDMA0) & (1 << USPI_MASTER_TX_DMA_CH)) == 0);

    /* Clear the PDMA transfer done flag */
    PDMA_CLR_TD_FLAG(PDMA0, (1 << USPI_MASTER_TX_DMA_CH));
...
static void _Open_PDMA(void)
{
    ...
    /* Enable PDMA channel */
    PDMA_Open(PDMA0, (1 << USPI_MASTER_TX_DMA_CH));

    /* Set transfer width (8 bits) */
    PDMA0->DSCT[USPI_MASTER_TX_DMA_CH].CTL = (PDMA0->DSCT[USPI_MASTER_TX_DMA_CH].CTL & ~PDMA_DSCT_CTL_TXWIDTH_Msk) | PDMA_WIDTH_8;
    /* Set destination address */
    PDMA_SET_DST_ADDR(PDMA0, USPI_MASTER_TX_DMA_CH, (uint32_t)&SPI_LCD_PORT->TXDAT);
    /* Set source/destination attributes */
    PDMA0->DSCT[USPI_MASTER_TX_DMA_CH].CTL = (PDMA0->DSCT[USPI_MASTER_TX_DMA_CH].CTL & ~(PDMA_DSCT_CTL_SAINC_Msk | PDMA_DSCT_CTL_DAINC_Msk)) | (PDMA_SAR_INC | PDMA_DAR_FIX);
    ...
    PDMA0->DSCT[USPI_MASTER_TX_DMA_CH].CTL |= PDMA_TBINTDIS_DISABLE;
}
