 ...
 * @file     NuBL2_main.c
 * @version  V1.00
 * @brief    Demonstrate to do secure OTA update for NuBL32 firmware by NuBL2 with MKROM library.
 *
...
volatile uint32_t gNuBL2_32Key[8];
...
int main(void)
{
    ...
    /* normal boot */
    /* verify firmware identity and integrity */
    NuBL2_Init();
...
_VERIFY_FAIL:
    ...
    if (OTA_TaskProcess() == 0)
    {
        ...
        printf("OTA update for BL3x verify failed: SYS:%d\n", FMC_Read(SYS_FW_OTA_STATUS_BASE));
        if (FMC_Read(SYS_FW_OTA_STATUS_BASE) == 1)
        {
            u32NeedReset = TRUE;
        }
...
_VERIFY_FAIL:
    ...
    {
        /* check OTA status and re-boot for update firmware */
        printf("OTA update for NuBL3x verify failed: SYS:%d\n", FMC_Read(SYS_FW_OTA_STATUS_BASE));
        if (FMC_Read(SYS_FW_OTA_STATUS_BASE) == 1)
        {
            u32NeedReset = TRUE;
        }
