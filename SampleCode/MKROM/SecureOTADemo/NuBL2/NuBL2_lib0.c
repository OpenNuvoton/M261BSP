...
/**
  * @brief      Perform NuBL2 to verify NuBL32 F/W
  * @param[in]  mode        bit-0: 0:verify NuBL32 \n
  *                         bit-4: 0:Integrity check; 1:Identify check
  * @retval     0           Success
  * @retval     others      Failed
  * @details    This function is used to perform identify, authenticate and verify F/W integrity of NuBL32. \n
  *             Flow: \n
...
int32_t NuBL2_ExecuteVerifyNuBL3x(uint32_t *buf, int32_t mode)
{
    ...
    if(memcmp(&Hash[0], &HashValue[0], (HASH_SIZE/8)) != 0)
    {
        uint32_t *tmp;
        tmp = (uint32_t *)&Hash;
        NUBL_MSG("Verify NuBL32 F/W Hash [FAIL]\n\n");
        NUBL_MSG("F/W Hash:\n");
    ...
    NUBL_MSG("Verify NuBL32 Key Storage Hash [PASS]\n\n");
...
/* Step 1. Decrypt NuBL3x public key */
    ...
    NUBL_MSG("Decrypt NuBL3 public key [Done]\n\n");
...
/* Step 3. Identify NuBL3x public key (encloed in Key Storage and F/W info) */
    ...
    {
        NUBL_MSG("Identify NuBL32 public key [FAIL]\n\n");
        ret = -3001;
        goto _exit_NuBL2_ExecuteVerifyNuBL3x;
    }
#else
...
/* Step 4. Authenticate NuBL3x info Hash */
    ...
    if(NuBL2_VerifyNuBL3xECDSASignature((uint32_t *)Hash, (ECDSA_SIGN_T *)&FwInfo.sign.au32R[0], (ECC_PUBKEY_T *)&PubKey.au32Key0[0], mode) != 0)
    {
        ...
        NUBL_MSG("Authenticate NuBL32 info signature [FAIL]\n\n");
    ...
    NUBL_MSG("Authenticate NuBL32 info signature [PASS]\n\n");
...
/* Step 5. Verify NuBL3x F/W Hash */
    if((mode&BIT4) != BIT4) /* if BIT4 == 0, dnubo not check F/W integrity */
    {
        ...
        NUBL_MSG("Verify NuBL32 F/W Hash [PASS]\n\n");
    }
    ret = 0;
...
*/
int32_t NuBL2_GetNuBL3xECDHKeys(uint32_t Key32[], uint32_t Reserved[])
{
    volatile int32_t    i, ret = -1000;
    ...
    NUBL_MSG("\nCalculate (NuBL2*NuBL32) ECDH keys.\n\n");
...
int32_t NuBL2_VerifyNuBL3xECDSASignature(uint32_t *msg, ECDSA_SIGN_T *sign, ECC_PUBKEY_T *pubkey, int32_t mode)
{
    ...
    NUBL_MSG("\nNuBL2 verify NuBL32 FW info signature.\n");
...
/**
  ...
  * @retval     1,2,3       Success, Bit0:NuBL32
  * @retval     -1      Failed
  * @details    This function is used to perform identify, authenticate and verify F/W integrity of NuBL32. \n
  *             Flow: \n
  *                 1. Decrypt(use NuBL2 AES Key )Key Storage to get NuBL32x public key \n
