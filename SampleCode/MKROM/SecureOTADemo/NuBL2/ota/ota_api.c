...
/**
  ...
  * @param[in]  u8NuBLxSel  NuBL3x firmware package selection. Bit0 = 1: NuBL32
  * @retval     0           Success
  * @retval     others      Failed
...
uint8_t OTA_API_SDFwPackWriteOpen(uint8_t u8NuBLxSel)
{
    ...
    if(res != FR_OK)
    {
        ...
        {
            printf("new NuBL32 FW package error!(0x%x)\n", res);
        }
    }
    ...
    if(res != FR_OK)
    {
        printf("open NuBL32 FW package error!(0x%x)\n", res);
    }
    return res;
}
...
uint8_t OTA_API_SDClose(uint8_t u8NuBLxSel)
{
    ...
    if(res != FR_OK)
    {
        printf("Close NuBL32 FW package error!(0x%x)\n", res);
    }
